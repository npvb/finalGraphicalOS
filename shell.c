...
void doCreateFile(char* line)
{
	...
	while(c!=0x3)//poner que sea diferente de un menor
	{
		c=getchar();
		file[index++]=c;
		if (c!=0x3)
...
void doRemove(char* line)
{
	char* nombre=getargument(line);
 	rm(nombre);
}

void doList(char* line)
{
	char* nombre=getargument(line);
	List(nombre);
}
...
void doshell()
{
	...
	while(1==1)
	{
		...
		if (line[0]==0xd)
			...
			doList(line);	
		else if (iscommand(line,"CAT\0")==1)
				doEcho(line);
		else if (iscommand(line,"cat\0")==1)
				doEcho(line);
		else if (iscommand(line,"ECHO\0")==1)
				doCreateFile(line);	
		else if (iscommand(line,"echo\0")==1)
			doCreateFile(line);
		else
